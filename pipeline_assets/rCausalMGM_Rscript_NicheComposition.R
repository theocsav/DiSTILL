...
run_analysis <- function(data_frame, feature_vars, output_dir, prefix, num_boots, seed = 42) {
  ...
  for (target in target_vars) {
    ...
    if (identical(target, "Disease.Health.State")) {
      dot_path <- file.path(output_dir, "rcausal_graphviz_niche_disease_state.dot")
      png_path <- file.path(output_dir, "rcausal_graphviz_niche_disease_state.png")
      write_clean_dot_from_sif(
        fci_sif_path,
        dot_path,
        "Niche Disease State Graph"
      )
      if (!render_dot_png(dot_path, png_path)) {
        ...
        plot(
          ...
          nodeAttr = NODE_ATTR,
          edgeAttr = EDGE_ATTR,
          main = "Niche Disease State Graph"
        )
        dev.off()
      }
    }
...
niche_data <- read.csv(args$input_file)
niche_data <- niche_data %>% column_to_rownames("field_of_view")
feature_vars <- grep("^Niche_", colnames(niche_data), value = TRUE)
run_analysis(niche_data, feature_vars, args$output_dir, "Niches", args$num_boots, args$seed)
