...
cr_path = os.path.join(output_dir, 'classification_report.txt')
...
with open(log_file_path, 'w') as f:
    ...
    random_search = RandomizedSearchCV(
        estimator=pipe, param_distributions=param_distributions, n_iter=200, cv=sgkf,
    ...
    # NEW: Wrap the loop with tqdm() for a progress bar
    ...
    with open(cr_path, 'w') as cr_file:
        cr_file.write(classification_report(all_y_true, all_y_pred, zero_division=0))

    # Save the confusion matrix to a file
    cm_df.to_csv(cm_csv_path)

    # Save a plot of the confusion matrix to a file
    ...
    plt.tight_layout()
    plt.savefig(cm_png_path, dpi=600)
