...
This is the canonical execution path for the Distill app. Legacy one-off scripts
under `pipeline_assets/` remain in the repo for reference, but new reproducible
runs should be expressed as presets and launched through `run_pipeline.py`.
...
For an app-wide audit of canonical versus legacy entrypoints, see `docs/DISTILL_PIPELINE_AUDIT.md`.
...
Large HPG runs can also be split into:

- GPU `cell2loc`
- CPU `nmf`, `post_nmf`, `rcausal_mgm`, `mlp`, `report`

This split is useful when `cell2location` benefits from CUDA but downstream
stages do not.
